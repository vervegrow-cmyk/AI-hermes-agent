...
class VideoCompose(BaseTool):
    ...
    def _remotion_render(self, inputs: dict[str, Any]) -> ToolResult:
        ...
        # Remotion renders in a browser context, which cannot reliably load
        # arbitrary local file:/// assets on Windows. Stage runtime assets into
        # remotion-composer/public and reference them via staticFile().
        runtime_public_dir = composer_dir / "public" / "_runtime_assets" / output_path.stem
        runtime_public_dir.mkdir(parents=True, exist_ok=True)

        # Convert local file paths into Remotion-public relative assets.
        for cut in props.get("cuts", []):
            source = cut.get("source", "")
            if not source or source.startswith(("http://", "https://", "data:")):
                continue

            normalized = source.replace("file:///", "").replace("file://", "")
            resolved = Path(normalized).resolve()
            if not resolved.exists():
                continue

            target_name = f"{cut.get('id', 'asset')}{resolved.suffix.lower()}"
            staged = runtime_public_dir / target_name
            if not staged.exists():
                shutil.copy2(resolved, staged)
            cut["source"] = f"_runtime_assets/{output_path.stem}/{target_name}".replace("\\", "/")
