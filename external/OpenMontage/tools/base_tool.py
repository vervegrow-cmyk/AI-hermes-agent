...
def _load_dotenv() -> None:
    ...
    Search the repo-local .env first, then walk upward so nested workspaces can
    inherit a shared parent .env such as AI-hermes-agent/.env.
    """
    import re
    ...
    for base in [project_root, *project_root.parents]:
        env_path = base / ".env"
        if env_path.is_file() and env_path not in candidate_paths:
