...
exports.bakeVersionstamp = exports.range = exports.unpack = exports.name = exports.packUnboundVersionstamp = exports.pack = exports.rawRange = exports.unboundVersionstamp = exports.concat2 = void 0;
...
(function (Code) {
    ...
    Code[Code["Bytes"] = 1] = "Bytes";
    Code[Code["String"] = 2] = "String";
    Code[Code["Nested"] = 5] = "Nested";
    Code[Code["IntZero"] = 20] = "IntZero";
    Code[Code["PosIntEnd"] = 29] = "PosIntEnd";
    Code[Code["NegIntStart"] = 11] = "NegIntStart";
    Code[Code["Float"] = 32] = "Float";
    Code[Code["Double"] = 33] = "Double";
    Code[Code["False"] = 38] = "False";
    Code[Code["True"] = 39] = "True";
    Code[Code["UUID"] = 48] = "UUID";
    // There's also an 80 bit versionstamp, but none of the other bindings use it.
    Code[Code["Versionstamp"] = 51] = "Versionstamp";
})(Code || (Code = {}));
/** This helper only exists because {type: 'unbound versionstamp'} is so awful to type. */
exports.unboundVersionstamp = (code) => ({ type: 'unbound versionstamp', code });
...
class BufferBuilder {
    ...
    appendByte(val) { this.need(1); this.storage[this.used++] = val; }
    appendString(val) {
...
const adjustFloat = (data, isEncode) => {
...
const encode = (into, item, versionstampPos) => {
    if (item === undefined)
        ...
        if (versionstampPos.stamp != null)
            throw new TypeError('Tuples may only contain 1 unset versionstamp');
        ...
        let x = item; // Compile error if this is legitimately reachable
        throw new TypeError('Packed items must be basic types or lists');
    }
};
function packRaw(arr) {
    ...
    if (Array.isArray(arr)) {
        ...
        encode(builder, arr, versionstampPos);
...
exports.pack = (arr) => {
    ...
    if (!Buffer.isBuffer(pack))
        throw new TypeError('Incomplete versionstamp included in vanilla tuple pack');
    return pack;
};
/**
 ...
 * versionstamp.
 */
exports.packUnboundVersionstamp = (arr) => {
    const pack = packRaw(arr);
    if (Buffer.isBuffer(pack))
        throw new TypeError('No incomplete versionstamp included in tuple pack with versionstamp');
    return pack;
};
// *** Decode
function decodeBigInt(buf, offset, numBytes, isNeg) {
    let num = BigInt(0);
    let shift = 0;
...
function decodeNumberOrBigInt(buf, offset, numBytes, isNeg) {
...
function decode(buf, pos, vsAt, noCanonicalize) {
    const code = buf.readUInt8(pos.p++);
    let { p } = pos;
    switch (code) {
        ...
        case Code.Nested: {
            ...
            while (true) {
                if (buf[pos.p] === 0) {
                    if (pos.p + 1 >= buf.length || buf[pos.p + 1] !== 0xff)
                        break;
                    else {
                        pos.p += 2;
                        result.push(null);
                    ...
                    result.push(decode(buf, pos, vsAt, noCanonicalize));
            }
 ...
 */
function unpack(buf, noCanonicalize = false) {
    const pos = { p: 0 };
    const arr = [];
 ...
 */
function range(arr) {
    var packed = exports.pack(arr);
    return {
        begin: exports.concat2(packed, exports.rawRange.begin),
        end: exports.concat2(packed, exports.rawRange.end),
    };
}
exports.range = range;
const vsFrom = (versionstamp, code) => {
...
function bakeVersionstamp(val, versionstamp, codeBytes) {
    ...
    for (let i = 0; i < val.length; i++) {
        ...
        if (Array.isArray(v))
            ...
            val[i] = {
                type: 'versionstamp',
                value: codeBytes ? exports.concat2(versionstamp, codeBytes) : vsFrom(versionstamp, v.code)
            };
        }
    }
}
exports.bakeVersionstamp = bakeVersionstamp;
