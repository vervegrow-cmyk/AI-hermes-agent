...
这是对 [`bigint-buffer`](https://www.npmjs.com/package/bigint-buffer) 的安全加固 fork，用于在不改上游依赖的情况下，通过 **npm overrides** 替换掉依赖树中的 `bigint-buffer@1.1.5`。

## 用法（推荐）

在你的应用项目（根 `package.json`）里加入：
...
这样所有依赖 `bigint-buffer` 的包都会被替换到本包（目录名仍会是 `node_modules/bigint-buffer`，所以 `require('bigint-buffer')` 仍正常）。

## 说明

- 本包保留原包 API：`toBigIntLE/toBigIntBE/toBufferLE/toBufferBE`
- 原生实现增加了输入校验、边界检查与更安全的位运算处理；同时 JS 层也做了兜底校验
- 由于本包用于被 overrides 引用，尽量不携带 devDependencies，避免把测试/构建工具引入业务依赖树
