...
$opencliServer = Join-Path $repoRoot "tools\mcp\opencli_mcp_server.py"

function Start-OpenCliMcp {
    $existing = Get-CimInstance Win32_Process | Where-Object {
        $_.CommandLine -like '*opencli_mcp_server.py*' -and $_.ProcessId -ne $PID
    }
    ...
    $command = "`$env:HERMES_MCP_TRANSPORT='http'; `$env:HERMES_MCP_PORT='8093'; Set-Location '$repoRoot'; python '$opencliServer'"
...
if ($dockerReady) {
    ...
    if ($openhandsReady) {
        ...
        try {
            python .\scripts\register_openhands_opencli_mcp.py | Out-Null
            $registered = $true
        }
        finally {
            Pop-Location
...
[pscustomobject]@{
    opencli_mcp_pid = $mcpPid
    docker_ready = $dockerReady
    openhands_ready = $openhandsReady
    opencli_registered_in_openhands = $registered
